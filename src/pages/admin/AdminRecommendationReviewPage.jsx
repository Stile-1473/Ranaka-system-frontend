...
const getWorkflowIndex = (request) => {
  ...
  if (request.status === "AUTHORIZED" || request.status === "COMPLETED") {
    return adminWorkflowSteps.length - 1;
  }

  const activeIndex = adminWorkflowSteps.findIndex(
...
function AdminRecommendationReviewPage() {
  ...
  const recommendExistingRequest = useApprovalMutationStore(
    (state) => state.recommendExistingRequest
  );
  ...
  const recommendStatus = useApprovalMutationStore((state) => state.recommendStatus);
  ...
  const handleAction = async (action) => {
    ...
    try {
      if (action === "recommend") {
        await recommendExistingRequest(requestId, { comment: trimmedComment || null });
        toast.success("Request recommended", {
          description: "The request has moved to GM approval.",
        });
      }
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
                Review the procurement request, confirm that the details are complete, and decide whether it should move forward, be returned, or be rejected.
              </p>
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.65fr_0.85fr]">
        <div className="space-y-6">
          ...
          <SectionBlock
            title="Overview"
            description="Core request context for recommendation review."
          >
        ...
        <div className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          <SectionBlock
            ...
            description="Leave a clear note, then choose the action that matches the quality and readiness of this request."
          >
            <TextareaField
              label="Decision Comment"
              placeholder="Add your recommendation note, correction reason, or rejection reason..."
            ...
            <div className="mt-5 space-y-3">
              <DecisionButton
                ...
                label="Recommend to GM"
                helper="Use this when the request is complete and ready for management approval."
                tone="success"
                disabled={busy}
              ...
              <DecisionButton
                ...
                helper="Send it back when details, costing, or supporting information need attention."
                tone="warning"
                disabled={busy}
...
export default AdminRecommendationReviewPage;
