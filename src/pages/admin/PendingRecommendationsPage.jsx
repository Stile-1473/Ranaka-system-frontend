...
function PendingRecommendationsPage() {
  ...
  const stats = useMemo(
    () => ({
      ...
      highValue: queue.filter((request) => Number(request.estimatedCost || 0) >= 5000)
        .length,
    }),
    [queue]
  );
    ...
    <div className="space-y-6">
      <div className="page-action-bar">
        <div className="page-action-copy">
          <p className="section-title">Admin Queue</p>
          ...
          <p className="page-action-subtitle">
            Review submitted requests, prioritise urgent items, and move valid work forward to management approval.
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile label="High Value" value={stats.highValue} />
      </div>
              ...
              <div className="px-6 py-12">
                <div className="rounded-[1.35rem] border border-dashed border-white/10 bg-white/[0.03] px-5 py-10 text-center">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-400">
                    There are no recommendations matching the current search and filter.
                  </p>
                </div>
              </div>
            ) : (
